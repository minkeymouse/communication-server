...
export class MessageRepository {
  ...
  public searchMessages(agentId: string, query?: string, limit?: number): Message[] {
    const filters: {
      fromAgentId?: string;
      toAgentId?: string;
      state?: string;
  ...
  public getMessagesByConversation(conversationId: string): Message[] {
    try {
      const { query, params } = QueryBuilder.buildMessageSearchQuery({
        limit: 1000 // Get all messages in conversation
      });
      const stmt = this.db.prepare(query.replace('WHERE', 'WHERE conversation_id = ? AND'));
      const rows = stmt.all(conversationId, ...params) as any[];
      return rows.map(row => this.mapRowToMessage(row));
    } catch (error) {
  ...
  public archiveMessage(messageId: string): boolean {
    try {
      const stmt = this.db.prepare(`
        UPDATE messages 
        SET archived_at = ?, state = 'archived' 
        WHERE id = ?
      `);
      const result = stmt.run(new Date().toISOString(), messageId);
      return result.changes > 0;
  ...
  public getUnreadMessageCountForAgent(agentId: string): number {
    try {
      const stmt = this.db.prepare('SELECT COUNT(*) as count FROM messages WHERE to_agent_id = ? AND is_read = 0');
      const result = stmt.get(agentId) as any;
      return result ? result.count : 0;
    } catch (error) {
  ...
  public getMessagesByState(agentId: string, state: string): Message[] {
    try {
      const { query, params } = QueryBuilder.buildMessageSearchQuery({
        toAgentId: agentId,
        state,
      ...
      console.error('Error getting messages by state:', error);
      return [];
    }
  }
  ...
  public getRecentMessages(limit: number = 50): Message[] {
    try {
      const { query, params } = QueryBuilder.buildMessageSearchQuery({ limit });
      const stmt = this.db.prepare(query);
      const rows = stmt.all(...params) as any[];
      return rows.map(row => this.mapRowToMessage(row));
    } catch (error) {
  ...
  public getMessagesInDateRange(startDate: Date, endDate: Date): Message[] {
    try {
      ...
      console.error('Error getting messages in date range:', error);
      return [];
    }
  }
  ...
  public getMessagesRequiringReply(agentId: string): Message[] {
    try {
      ...
      const stmt = this.db.prepare(query.replace('WHERE', 'WHERE requires_reply = 1 AND'));
      const rows = stmt.all(...params) as any[];
      return rows.map(row => this.mapRowToMessage(row));
    } catch (error) {
      console.error('Error getting messages requiring reply:', error);
      return [];
    }
  }
